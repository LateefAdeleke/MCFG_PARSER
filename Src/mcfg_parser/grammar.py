...
Classes
----------
- MCFGRuleElement: This class represents a single part of an MCFG rule.
- MCFGRuleElementInstance: This class repreasents an instantiated version of the  of an MCFGRuleElement
- MCFGRule: Represents a complete rule of an  MCFG.
"""
...
class MCFGGrammar:
    ...
    def __init__(self, rules: set[MCFGRule], start_symbol: str):
        self._rules = rules
        self._start_symbol = start_symbol
        self._rules_indexed = defaultdict(set)
        self._non_terminal_dimensions = defaultdict(int)

        self._index_rules()
        self._validate_rules()

    def _index_rules(self):
        """Index rules by their left-side non-terminal for fast retrieval."""
        for rule in self._rules:
            left_variable = rule.left_side.variable
            self._rules_indexed[left_variable].add(rule)

            # Keep track of the dimension of each non-terminal
            dimension = len(rule.left_side.string_variables)
            self._non_terminal_dimensions[left_variable] = dimension

    def _validate_rules(self):
        """Validate rules and ensure consistency with MCFG constraints."""
        for rule in self._rules:
            # Check for consistency between rule dimensions
            left_dimension = len(rule.left_side.string_variables)
            right_dimensions = [len(elem.string_variables) for elem in rule.right_side]

            if any(d <= 0 for d in [left_dimension] + right_dimensions):
                raise ValueError(f"Invalid dimensions in rule: {rule}")

            # Ensure non-terminals are consistent in dimensions
            for elem in rule.right_side:
                var = elem.variable
                if var in self._non_terminal_dimensions:
                    expected_dimension = self._non_terminal_dimensions[var]
                    actual_dimension = len(elem.string_variables)
                    if expected_dimension != actual_dimension:
                        raise ValueError(
                            f"Inconsistent dimension for non-terminal {var} in rule: {rule}"
                        )

    def get_rules_by_left_side(self, variable: str) -> Set[MCFGRule]:
        """Retrieve rules given a specific left-side non-terminal."""
        return self._rules_indexed.get(variable, set())

    @property
    def start_symbol(self) -> str:
        return self._start_symbol

    def yieldable(self) -> Set[str]:
        """Return the set of all yieldable non-terminals."""
        return set(self._non_terminal_dimensions.keys())

    @classmethod
    def from_string(cls, grammar_definition: str, start_variable: str = 'S') -> 'MCFGGrammar':
        """
        Creates a grammar from a multiline string, each line representing a rule.

        Parameters:
        - grammar_definition: str, multiline string with each rule on a separate line.
        - start_variable: str, the start symbol of the grammar.

        Returns:
        - An instance of MCFGGrammar.
        """
        rule_lines = grammar_definition.strip().split('\n')
        rules = {MCFGRule.from_string(line.strip()) for line in rule_lines if line.strip()}

        return cls(rules, start_variable)